...
nrow(cdc[which(cdc$gender == "m"), ])
nrow(cdc[which(cdc$genhlth == "excellent"), ])/nrow(cdc)
```
...
```Number of males who smoked atleast 100 cigarettes in their lifetime is higher than female participants.```
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    ```This box plot shows participants in excellent health conditon has better 
    (lower value) BMI than   other category.  
    Another interesting variable to relate to BMI is gender. The general health
    condition and eating habits of gender are connected to BMI. The figure shows
    females have better BMI than men.```
    ```{r}
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    ```{r}
    plot(cdc$weight, cdc$wtdesire, main="Weight Vs. Desired Weight")
    ```
    There is a negative relationship. Looking at points around 200 pound in 
    `weight`, the `desired weight` is less than 200.
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    typeof(wdiff)
    ```
    ```wdiff = 0 ==> Person is happy with current weight  
    wdiff positive ==> Person wants to gain weight  
    wdiff negative ==> Person wants to loose weight```
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    summary(wdiff)
    ```
    3rd quartile is at 0. It means 75% of people wants to loose weight. They 
    think current weight is higher than expected weight. 
...
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    ...
    summary(cdc[which(cdc$gender == "m"), ]$wdiff)
    summary(cdc[which(cdc$gender == "f"), ]$wdiff)
    ```
    Considering mean and median men tend to loose less weight than women. On 
    avergae women wants loose 18.8 pounds where as men wants to loose only
    10.71 pounds.
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    length(cdc$weight[abs(cdc$weight - mean_weight) <= sd_weight])/nrow(cdc)
    ```
