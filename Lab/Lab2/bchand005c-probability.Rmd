...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    ```
    Streak of 1 means a hit followed by a miss (1H and 1M). 
    Streak of 0 means no hit but a miss.
    ```
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    ```
    I expect the graph to be right skewed but at the tail side, it can be 
    totally different. This is because of the variability in data. It is caused 
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    ```
    They both look similar. Since the pattern of kobe's shooting resembles the 
    simulated shooter, there is no evidence that the hot hand model fits Kolbe's
    shooting pattern.
    ```
