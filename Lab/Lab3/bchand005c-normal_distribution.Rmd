...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    ```
    ```
    Both of the distributions are unimodal and fairly symmetric (normal).
    ```
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?
    ```
    Yes, the data follows a nearly normal distribution.
    ```
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    ```
    ```
    Not all points fall on the line. There are some outliers at the higher end.
    Both plots have most of the points fall on the line.
    ```
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    ```
    Yes, they look similar. They do provide evidence that the female heights
    are nearly normal.
    ```
...
5.  Using the same technique, determine whether or not female weights appear to 
    ...
    ```
    ```
    The plots shows that distribution is right skewed which contradicts with 
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    ```
    What is the probability that a randomly chosen young adult female has height
    between 170 cm and 180 cm?
    ```
    ```{r}
    pnorm(q = 180, mean = fhgtmean, sd = fhgtsd) - pnorm(q = 170, mean = fhgtmean, sd = fhgtsd)
    sum(fdims$hgt > 170 & fdims$hgt <= 180) / length(fdims$hgt)
    ```
    ```
    What is the probability that a randomly chosen young adult female has weight
    above 50 pounds?
    ```
    ```{r}
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    ```
    These plots doesn't follow normal distribution model. D is right skewed.
    ```
