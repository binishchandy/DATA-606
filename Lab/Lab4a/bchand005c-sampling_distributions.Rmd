...
```
```
The distribution is fairly normal but slightly skewed to the right.
```
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    ```
    ```
    The sample distribution is also fairly normal but slightly skewed to right.
    ```
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    ```
    ```
    samp1 and samp2 means are different but fairly close to population mean but 
    not really close. But when the sample size increased the mean tends to fall 
    very close to population mean.
    ```
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    ```
    ```
    The sampling distribution fairly looks like normal distribution with sample
    mean ie center coniciding with population mean. If 50,000 sample means are
    collected, the curve will become more normal and the varialbility decreases
    as SD decreases.
    ```
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    ```
    ```
    sample_means_small elements represent sample mean of 100 random samples of 
    sample size 50.
    ```
...
```
The center coincides more with the true population mean and the spread 
decreases as variability in data decreases.
```
...
-   Since you have access to the population, simulate the sampling 
    ...
    ```
    ```
     Because the sample mean is an unbiased estimator, the sampling distribution 
     is centered close to the true average housing price of the the population
    ```
    ```{r}
     mean(price)
    ```
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    be the mean sale price of homes in Ames?
     ...
     The variability or spread decreases when the sample size increased.  
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread?  If
    ...
    ```
    More spread is for distribution with less samples ie 2. We prefer small
    spread.
    ```
