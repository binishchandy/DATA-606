...
```
```
Sample distribution is fairly normal with some skew to right with couple of 
outliers.
"typical" means the average value of the sample.
```

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?
```
It won't be identical as the sample is randomly selected. It would be similar
as the samples are drawn from same population so the samples tend to follow the 
...
The sample size is large ie n > 30
...
```
We are 95% confident that the population parameter is between a 
...
-   Using the following function (which was downloaded with the data set), 
    ...
    for (i in 1:50) {
            if (between(mean(population), lower_vector[i], upper_vector[i])) {
                    confidence_interval_count = confidence_interval_count + 1
            }
    }
    confidence_interval_count / 50
    ```
    ```
    It is not exactly equal. It is because there is still a 5% chance for the 
    range of values to miss true population parameter.
    ```

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    ```
    99% confidence interval has z* value of 2.58
    ```
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    ```
    ```
    The percentage is relatively very closer to the confidence interval as we 
    now have a wide range of vaues than previous confidence interval.
    ```
