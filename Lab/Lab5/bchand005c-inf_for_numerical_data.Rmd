...
```
The cases are observations about birth details of babies born in NC. 
```
```{r}
nrow(nc)
```
...
```{r}
boxplot(weight~habit, data=nc, ylab="Weight of child") 
```
```
Plot shows the median of two distributions differ.
```
...
`length`.
```
Independece : The sample is random set of observations from population.
```
```{r}
hist(nc$weight)
...
```
```
Normal distribution : Fairly normal with slight left skew and fairly large
sample size. So skew can be ignored.
```

4.  Write the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different.
```
u = u(wt_of_nonsmoking) - u(wt_of_smoking)
H0: u = 0
HA: u != 0
```
...
mothers. Use a method of your choice, and explain how your method works.
    ...
    ```
    ```
      34 is the cutoff age. I used dplyr package to group by based on mature
      column values and found max and min ages.
    ```
...
plain language.
   ```
     Test if the average pregnancy weeks for smoking and non-smoking mothers 
     are different.
     u = u(weeks_of_nonsmoking) - u(weeks_of_smoking)
     H0: u = 0
     HA: u != 0
   ```
   ```{r, eval=TRUE, tidy=FALSE}
   ...
   ```
   ```
    Since p > 0.05, we cannot reject H0. There is no conclusive evidence to 
    show the average preganancy weeks for non smoking and smoking mothers are
    different.
   ```
