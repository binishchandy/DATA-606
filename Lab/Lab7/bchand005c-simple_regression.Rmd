...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    ```
    Scatter plot can be used to dislplay the relationship.
    ```
    ```{r}
    plot(mlb11$at_bats, mlb11$runs)
    ```
    ```
    The relationship looks linear. I would be comfortable using a linear model 
    to predict the number of runs.
    ```
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    ```
    There is a a linear relation between at_bats and runs variable. It is a 
    direct linear relationship that has a good correlation (~0.6). There are some 
    outliers but they are not significant.
    ```
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    Call:
        ...
        Sum of Squares:  141598
    
    Smallest Sum of Squares:  131686.6
    ```
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    ...
    \]
    ```
    Homeruns have a strong positive correlation with runs scored. For every 
    homerun scored, the team’s run count increases by 1.8345 (slope of the 
    regression line). In other words, every homerun helps bring in an additional
    0.8345 run.
    ```
...
5.  If a team manager saw the least squares regression line and not the actual 
    ...
    \]
    ```{r}
       (runs_est <- -2789.2429 + 0.6305 * 5578)
       (runs_observed <- subset(mlb11, at_bats==5579, select = runs)$runs)
       (residual <- runs_observed - runs_est)
    ```
    ```
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    ```
    Residual plot shows no obvious patterns. It is reasonalbe to to try to fit
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    ```
    Yes, it meets the condition. The plot is fairly normal with slight right skew.
    ```
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    be met?
    ```
    From the residual plot, the variability of data remains almost constant
    around line at zero.
    ```
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    tell?
    ```{r}
    (r_runs_batavg <- cor(mlb11$runs, mlb11$bat_avg))
    (r_runs_atbats <- cor(mlb11$runs, mlb11$at_bats))
    (r_sq_runs_batavg <- r_runs_batavg ^ 2)
    (r_sq_runs_atbats <- r_runs_atbats ^ 2)
    ```
    ```
    Strength of the linear fit is more explained by R^2. Closer the value of
    R^2, stronger linear fit.
    ```
    ```
    The bat_avg is a better predictor of runs than the at_bats variable. 
    R^2 measures the strength of the model in how well it explains the variability 
    of the response variables around the mean.
    ```
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    ```
    ```
    bat_avg has the highest R^2. The plots are calculations are already provided
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    ```
    ```
    new_obs seems to be the best predictor of runs.
    
    The all 3 new variables are far better models than bat_avg. There is strong
    correlation between runs and new_obs.
    ```
