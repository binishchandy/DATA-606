...
1.  Is this an observational study or an experiment? The original research
    ...
    ```
    This is an observational study. This can only show assosciation not causation. 
    Therefore, the original research question cannot be answered 
    with this study. A better question would be “Is there a relationship between 
    beauty and differences in course evaluations?”
    ```
...
2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    ...
    ```
    ```
    The distribution is unimodal with slight left skew. This tells that more 
    students gave high score to the courses. 
    This is what I expected to see because if the distribution is right skewed,
    there are lots of dissatisfied students at University of Texas.
    ```
...
3.  Excluding `score`, select two other variables and describe their relationship 
    ...
    mosaic plot).
    ```{r}
    plot(evals$age ~ evals$ethnicity, 
     xlab = "Ethnicity", ylab = "Age")
    ```
    ```
    Side-by-side boxplots shows the relationship between age and ethinicity.
    Median age of non minority teachers are higher than minority teachers. Also 
    the range of ages in both categories varies widely.
    ```
...
4.  Replot the scatterplot, but this time use the function `jitter()` on the
    ...
    ```
    ```
    Since lots of observations have same beauty and score values, they were 
    shown as single point on plot. With jitter, by adding some noise, they can be 
    shown separately. 
    ```
...
5.  Let's see if the apparent trend in the plot is something more than
    ...
    Yes, bty_avg is a statistically significant predictor of evaluation score 
    with p-value close of 0.
    It doesn't appear to be a practically significant predictor because for 
    every unit increase in bty_avg the model predicts only 0.06664 increase in 
    score.
    ```
...
6.  Use residual plots to evaluate whether the conditions of least squares
    ...
    ```
    ```
    Plot shows constant variability around 0. There are no apparent patterns. 
    ```
    ```{r}
    ...
    ```
    ```
    Histogram of the residuals is slightly left skewed. Since the data is collected
    without any random sampling ie more of volunteer survey, the response of a 
    student can be influenced by other students. So independence is not guaranteed.
    ```
...
7.  P-values and parameter estimates should only be trusted if the
    ...
    ```
    ```
    Nearly normal residuals : Slightly left skewed. But generally follows normal
    model.
...
8.  Is `bty_avg` still a significant predictor of `score`? Has the addition
    ...
    ```
    The parameter estimate for bty_avg has changed with addition of the gender 
    variable, but it remains significant predictor of score.
    ```
...
9.  What is the equation of the line corresponding to males? (*Hint:* For
    ...
    ```
    score = b0 + b1 * bty_avg + b2 * 1
          = 3.74734 + 0.07416×bty_avg + 0.17239 * 1
    For two professors who received the same beauty rating, evaluation score 
    from males tends to be higher.
    ```
...
10. Create a new model called `m_bty_rank` with `gender` removed and `rank` 
    ...
    ```
    ```
    R splits the rank variable with three levels into two variables.
    ```
...
11. Which variable would you expect to have the highest p-value in this model? 
    ...
    ```
    cls_profs
    Evaluations are for individual professors and their score should not depend 
    on the fact that other sections of the course had other professors.
    ```
...
12. Check your suspicions from the previous exercise. Include the model output
    in your response.
    ```
    I ran the model before answering Q.11
    ```

13. Interpret the coefficient associated with the ethnicity variable.
     ```
     With every other factor remains constant, the score will increase by 
     0.1234929 if the professor is not part of minority group.
     ```
...
14. Drop the variable with the highest p-value and re-fit the model. Did the
    ...
    ```
    ```
    The coefficients and significance for all variables changed with the removel of 
    cls_profs. This means that they are collinear with cls_profs.
    ```
...
15. Using backward-selection and p-value as the selection criterion,
    ...
    ```{r}
    m_b_best <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_credits + bty_avg + pic_color, data = evals)
    summary(m_b_best)
    ```
    ```
    score = 3.771922 + 0.167872 * ethnicity + 0.207112 * gender - 0.206178 * language
    - 0.006046 * age + 0.004656 * cls_perc_eval + 0.505306 * cls_credits + 
    0.051069 * bty_avg - 0.190579 * pic_color
    ```

16. Verify that the conditions for this model are reasonable using diagnostic 
    plots.
    ```{r}
    hist(m_b_best$residuals) 
    qqnorm(m_b_best$residuals)
    qqline(m_b_best$residuals)
    ```
...
17. The original paper describes how these data were gathered by taking a
    ...
    ```
    This can affect the assumption around independence of observations. If same
    student takes two coureses from same professor, one can affect the outcome
    of the other.
    ```
...
18. Based on your final model, describe the characteristics of a professor and 
    ...
    The professor should
     - be from a non minority background
     - be a male
     - have his degree from english speaking school
     - be younger
     - have a high percentage of students filling out evaluations
     - teaches one credit courses
     - have a high beauty ranking
     - have a black & white picture
    ```

19. Would you be comfortable generalizing your conclusions to apply to professors
    generally (at any university)? Why or why not?
    ```
    First of all this is a observational study, so I cannot make conclusions or
    generalizations. Also sample is taken from one university which is also bad.
    ```
